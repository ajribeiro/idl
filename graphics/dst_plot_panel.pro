...
;-
pro dst_plot_panel, xmaps, ymaps, xmap, ymap, $
...
if ~keyword_set(date) then begin
	caldat, dst_data.juls[0], month, day, year
...
if ~keyword_set(yrange) then $
	yrange = get_default_range('dst_index')
...
xtag = 'juls'
ytag = 'dst_index'
if ~tag_exists(dst_data, xtag) then begin
	prinfo, 'Parameter '+xtag+' does not exist in DST_DATA.'
	return
endif
if ~tag_exists(dst_data, ytag) then begin
	prinfo, 'Parameter '+ytag+' does not exist in DST_DATA.'
	return
endif
dd = execute('xdata = dst_data.'+xtag)
dd = execute('ydata = dst_data.'+ytag)

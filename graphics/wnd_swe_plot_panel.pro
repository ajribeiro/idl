...
;
; PURPOSE: 
; The procedure plots a panel of Wind SWE solar wind data.
; 
; CATEGORY: 
; Graphics
; 
...
; values are 'vx_gse','vy_gse','vz_gse','vt','np' and 'pd'.
;
...
;-
pro wnd_swe_plot_panel, xmaps, ymaps, xmap, ymap, $
...
if wnd_swe_info.nrecs eq 0L then begin
...
if ~keyword_set(date) then begin
	caldat, wnd_swe_data.juls[0], month, day, year
...
xtag = 'juls'
ytag = param
if ~tag_exists(wnd_swe_data, xtag) then begin
	prinfo, 'Parameter '+xtag+' does not exist in WND_SWE_DATA.'
	return
endif
if ~tag_exists(wnd_swe_data, ytag) then begin
	prinfo, 'Parameter '+ytag+' does not exist in WND_SWE_DATA.'
	return
endif
dd = execute('xdata = wnd_swe_data.'+xtag)
dd = execute('ydata = wnd_swe_data.'+ytag)

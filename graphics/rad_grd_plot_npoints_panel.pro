;+ 
; NAME: 
; RAD_GRD_PLOT_NPOINTS_PANEL
; 
; PURPOSE: 
; This procedure plots the time series of logarithm of the points 
; that survived the velocity gridding in a panel.
; 
; CATEGORY: 
; Graphics
; 
...
;-
pro rad_grd_plot_npoints_panel, xmaps, ymaps, xmap, ymap, $
...
xtag = 'mjuls'
ytag = 'vcnum'
if ~tag_exists((*rad_grd_data[int_hemi]), xtag) then begin
	prinfo, 'Parameter '+xtag+' does not exist in RAD_GRD_DATA.'
	return
endif
if ~tag_exists((*rad_grd_data[int_hemi]), ytag) then begin
	prinfo, 'Parameter '+ytag+' does not exist in RAD_GRD_DATA.'
	return
endif
dd = execute('xdata = (*rad_grd_data[int_hemi]).'+xtag)
dd = execute('ydata = (*rad_grd_data[int_hemi]).'+ytag)

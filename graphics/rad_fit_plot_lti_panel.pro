...
tmp = where(channel eq (*rad_fit_info[data_index]).channels, cc)
if cc eq 0L then begin
	prinfo, 'No data exists for channel '+strtrim(string(channel),2)+'. Choosing '+strtrim(string((*rad_fit_info[data_index]).channels[0]),2)
	channel = (*rad_fit_info[data_index]).channels[0]
endif
...
txdata = xdata
tchann = (*rad_fit_data[data_index]).channel
ttfreq = (*rad_fit_data[data_index]).tfreq
tscani = (*rad_fit_data[data_index]).scan_id
...
for i=0,nbeams-1 do begin
	for j=0,ngates-1 do begin		
		delta = abs(latitude - (*(*rad_fit_info[data_index]).fov_loc_center)[0,i,j])
...
for b=xrange[0], xrange[1] do begin
	...
	if n_elements(channel) ne 0 then begin
		data_inds = where((*rad_fit_data[data_index]).beam eq b and $
	...
	FOR d=0L, ndata_inds-2L DO BEGIN
		IF ydata[data_inds[d],gate[b]] ne 10000. THEN BEGIN
			...
			IF ~( ((*rad_fit_data[data_index]).gscatter[data_inds[d],gate(b)] EQ 0 AND scatterflag EQ 1) OR $
				((*rad_fit_data[data_index]).gscatter[data_inds[d],gate(b)] NE 0 AND scatterflag EQ 2)) THEN BEGIN
				color_ind = (MAX(where(lvl le ((ydata[data_inds[d],gate(b)] > scale[0]) < scale[1]))) > 0)
				IF param EQ 'velocity' AND scatterflag EQ 3 AND $
					(*rad_fit_data[data_index]).gscatter[data_inds[d],gate(b)] EQ 1 THEN $
						col = get_gray() $
				ELSE $
					col = cin[color_ind]
				POLYFILL, (*rad_fit_data[data_index]).beam[data_inds[d]]+[0,1,1,0], $

...
;
; PURPOSE: 
; The procedure plots a panel of Wind MAG solar wind data.
; 
; CATEGORY: 
; Graphics
; 
...
;-
pro wnd_mag_plot_panel, xmaps, ymaps, xmap, ymap, $
...
if wnd_mag_info.nrecs eq 0L then begin
...
if ~keyword_set(date) then begin
	caldat, wnd_mag_data.juls[0], month, day, year
...
xtag = 'juls'
ytag = param
if ~tag_exists(wnd_mag_data, xtag) then begin
	prinfo, 'Parameter '+xtag+' does not exist in WND_MAG_DATA.'
	return
endif
if ~tag_exists(wnd_mag_data, ytag) then begin
	prinfo, 'Parameter '+ytag+' does not exist in WND_MAG_DATA.'
	return
endif
dd = execute('xdata = wnd_mag_data.'+xtag)
dd = execute('ydata = wnd_mag_data.'+ytag)

...
;
; PURPOSE: 
; This function returns names of ACE SWE data files that fall in the given time interval.
; 
; CATEGORY: 
; Input/Output
; 
; CALLING SEQUENCE:
; Result = ACE_SWE_FIND_FILES(Date)
;
; INPUTS:
...
;
; PATH: Set this to a directory name in which to look for the ACE SWE files.
; The default is to use the output from ACE_SWE_GET_PATH(). If you set this
; variable, the files in that directory must follow the same naming convention
; as those in the global directory.
;
...
;-
function ace_swe_find_files, date, time=time, long=long, $
...
for i=0, no_d-1 do begin
	...
	if ~keyword_set(path) then $
		_path = ace_swe_get_path(yy)

	; check if directory exists
	if ~file_test(_path, /dir) then $
		err = 'Path to ACE SWE data does not exist: '+_path
	
	; find all zipped fitex files on the current day
	; ac_h0_swe_20090315_v10.cdf
	tfiles = file_search(_path+'/ac_h0_swe_'+astrdate+'_v*.cdf', count=fc)

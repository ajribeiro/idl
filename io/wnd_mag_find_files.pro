...
;
; PURPOSE: 
; This function returns names of Wind MAG data files that fall in the given time interval.
; 
; CATEGORY: 
; Input/Output
; 
; CALLING SEQUENCE:
; Result = WND_MAG_FIND_FILES(Date)
;
; INPUTS:
...
;-
function wnd_mag_find_files, date, time=time, long=long, $
...
for i=0, no_d-1 do begin
	...
	if ~keyword_set(path) then $
		_path = wnd_mag_get_path(yy)

	; check if directory exists
	if ~file_test(_path, /dir) then $
		err = 'Path to Wind MAG data does not exist: '+_path
	
	; find all zipped fitex files on the current day
	; wi_h0_mfi_20090315_v10.cdf
	tfiles = file_search(_path+'/wi_h0_mfi_'+astrdate+'_v*.cdf', count=fc)

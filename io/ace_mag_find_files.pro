...
;
; PURPOSE: 
; This function returns names of ACE MAG data files that fall in the given time interval.
; 
; CATEGORY: 
; Input/Output
; 
; CALLING SEQUENCE:
; Result = ACE_MAG_FIND_FILES(Date)
;
; INPUTS:
...
;
; PATH: Set this to a directory name in which to look for the ACE MAG files.
; The default is to use the output from ACE_MAG_GET_PATH(). If you set this
...
;-
function ace_mag_find_files, date, time=time, long=long, $
...
for i=0, no_d-1 do begin
	...
	if ~keyword_set(path) then $
		_path = ace_mag_get_path(yy)

	; check if directory exists
	if ~file_test(_path, /dir) then $
		err = 'Path to ACE MAG data does not exist: '+_path
	
	; find all zipped fitex files on the current day
	; ac_h0_swe_20090315_v10.cdf
	tfiles = file_search(_path+'/ac_h0_mfi_'+astrdate+'_v*.cdf', count=fc)

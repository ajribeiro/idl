;+ 
; NAME: 
; DMS_SSJ_CHECK_LOADED
;
; PURPOSE: 
; This function checks whether DMSP SSJ/4 data for the chosen time interval is
; already loaded.
; 
; CATEGORY: 
; Input/Output
; 
; CALLING SEQUENCE:
; Result = DMS_SSJ_CHECK_LOADED(Date, Sat)
;
; INPUTS:
...
;
; COMMON BLOCKS:
; DMS_SSJ_DATA_BLK: The common block holding the currently loaded radar data and 
...
;-
function dms_ssj_check_loaded, date, sat, time=time, long=long, $
	silent=silent
...
if dms_ssj_info.nrecs eq 0L then $
	return, !false

; check if right satellite
if dms_ssj_info.sat ne sat then $
	return, !false

;- give 3 minutes lee way
if dms_ssj_info.sjul gt sjul+3.d/1440.d then $
	return, !false

if dms_ssj_info.fjul lt fjul-3.d/1440.d then $
	return, !false
